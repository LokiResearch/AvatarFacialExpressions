...
public class EmoRay_Elastic : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Vector3 curPos = rayCursor.cursor.Position;
        Vector3 iconPos = selectedIcon.transform.position;

        float dist = (curPos - iconPos).magnitude;

        float intensity = Math.Min(dist * 3, 1);

        LineRenderer renderer = GetComponent<LineRenderer>();
        renderer.SetPositions(new Vector3[] { curPos, iconPos });
        renderer.startWidth = Mathf.Lerp(0.03f, 0.01f, intensity);
