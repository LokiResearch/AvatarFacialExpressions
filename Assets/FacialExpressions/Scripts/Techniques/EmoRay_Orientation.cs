...
public class EmoRay_Orientation : MonoBehaviour
{
    ...
    private Vector3 initialVectorUp = Vector3.zero;
    private Vector3 initialVectorForward = Vector3.zero;
    private void initIntensity()
    {
        initialVectorUp = VRInputsManager.GetMainHandController().GetTransform().up;
        initialVectorForward = VRInputsManager.GetMainHandController().GetTransform().forward;
        hapticIntensity.Reset(0);
    }
    private float intensity()
    {
        Vector3 currUp = VRInputsManager.GetMainHandController().GetTransform().up;
        Vector3 projectedUp = Vector3.ProjectOnPlane(currUp, initialVectorForward);
        float a = Vector3.Angle(initialVectorUp, projectedUp);
        float i = Mathf.Min(Mathf.Abs(a) / 50, 1f);
        // Debug.Log(i);
